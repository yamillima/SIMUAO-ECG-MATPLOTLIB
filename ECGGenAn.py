# -*- coding: utf-8 -*-
"""
Created on Thu Dec  6 14:13:58 2018
...
def ECG_NSFA(t,DesfaseP,DesfaseT,A,B,C,D,pi,TPW,TTW,TQRS,TPQ,TST,T,Aqrs,N):
    Signal=np.zeros([len(t),1],dtype=np.float64)
    for i in range(1,N):
        ANqrs=(2*Aqrs*T/((pi**2)*(i**2)*TQRS))*(1-math.cos(pi*i*TQRS/T))
        ANw=ANqrs
        for j in range (1,len(t)):
            SignTemp=ANw*math.cos(2*pi*i*t[j]/T)
            Signal[j]=Signal[j]+SignTemp;

    return Signal

def ECG_SFA(t,DesfaseP,DesfaseT,A,B,C,D,pi,TPW,TTW,TQRS,TPQ,TST,T,Ar1,As1,N):
    Signal=np.zeros([len(t),1],dtype=np.float64)
    for i in range (1,N):
...
def ECG_NSPL(t,DesfaseP,DesfaseT,A,B,C,D,pi,TPW,TTW,TQRS,TPQ,TST,T,Aqrs,Ap,At,N):
    Signal=np.zeros([len(t),1],dtype=np.float64)
    A0qrs=(-Aqrs*TQRS/T)
...
def ECG_SPL(t,DesfaseP,DesfaseT,A,B,C,D,pi,TPW,TTW,TQRS,TPQ,TST,T,Ap1,Ar1,As1,At1,N):
    Signal=np.zeros([len(t),1],dtype=np.float64)
...
def ECG_NSTP(t,DesfaseP,DesfaseT,A,B,C,D,pi,TPW,TTW,TQRS,TPQ,TST,T,Aqrs,Ap,At,N):
    Signal=np.zeros([len(t),1],dtype=np.float64)
    T=2*T/3
    for i in range(1,N):
        ANqrs=(2*Aqrs*T/((pi**2)*(i**2)*TQRS))*(1-math.cos(pi*i*TQRS/T))
...
def ECG_STP(t,DesfaseP,DesfaseT,A,B,C,D,pi,TPW,TTW,TQRS,TPQ,TST,T,Ap1,Ar1,As1,At1,N):
    Signal=np.zeros([len(t),1],dtype=np.float64)
    T=2*T/3
    for i in range (1,N):
        ANqrs1=(T/((pi**2)*(i**2)*TQRS))*((Ar1*(1-math.cos(pi*i*TQRS/T)))+As1*(math.cos(pi*i*TQRS/T)-1))
